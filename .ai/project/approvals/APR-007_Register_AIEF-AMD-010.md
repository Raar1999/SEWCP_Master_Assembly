# APR-007 — Registration of `AIEF-AMD-010` in the freeze registry
...
subject_path:  framework/AIEF-AMD-010_Stage_6_Constructions_and_Preflight_Dispositions.md
subject_hash:  486be10eb3bea89fb8c6c99949f1bb48e884cb556879e594cdd778dac5b0b829
...
`framework/AIEF-AMD-010_Stage_6_Constructions_and_Preflight_Dispositions.md`, at DC-1 normalised SHA-256
`486be10eb3bea89fb8c6c99949f1bb48e884cb556879e594cdd778dac5b0b829`.
...
AMD-21 rules the registration criterion: *an artifact is registered if it is an authorising instrument for a change to a frozen artifact, or the record of the authority under which such a change was made.* `AIEF-AMD-010` is the authorising instrument for the manifest change approved in `APR-006`. An unregistered authorising instrument is the exact exposure AMD-21 closed: the defence for a guarded artifact must itself be guarded, or it proves nothing (the APR-001/ECR-D-005 lesson).

## Consequences

| | |
|---|---|
| Registry membership | 25 → **26** |
| `framework/` coverage | 14 of 15 → **15 of 16** (all but the superseded `AIEF-ARCH-001`, ruled out by AMD-21) |
| Freeze-set aggregate | Recomputed under DC-2 over the 26-member registry; recorded in full in `FROZEN.md` § *Aggregate* and `STATE.frozen_set_hash` |
...
Applied by `chief-systems-engineer · S-2026-08-08-04` at the direction of the human owner. Under LAW-05 this session cannot verify its own work; the independent cold-context `qa-engineer` audit dispatched by the same instruction is the mitigating control.
