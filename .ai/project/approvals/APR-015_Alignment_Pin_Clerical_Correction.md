# APR-015 — Clerical correction of `spec/01` under ECR-D-001; geometric findings split out

> **Instance artifact.** Partition `project`. Schema `core/schemas/SCH-approval.schema.json`, severity BLOCKING.
> Satisfies the recorded-human-approval requirement of LAW-01 and LAW-10 for the repair of `VER-ECR-D-001` findings F1–F2, F5, F10.
...
supersedes:    APR-014                # voided by this edit under LAW-10; see below
ecr:           ECR-D-001
...
`APR-014` authorised ECR-D-001 disposition A and was correctly bound to
`spec/01_SEWCP-200_Cooling_Plate.md` at `a637ae18…7f8b9b54`. Independent verification
(`VER-ECR-D-001`) then returned **FAIL** and the repair requires a further edit to that
artifact — which **voids `APR-014` under LAW-10**, exactly as `APR-014` itself declares. This
approval supersedes it and binds to the corrected content. `APR-014` remains on the record as
...
enumerated option set. **No engineering value was set by an implementing agent**, which is the
property Option B was chosen to preserve.
