...
| Item | State |
|---|---|
| Lifecycle stage | `LC-M04` Implementation · gate `LC-M04-EXIT` — **computed, not asserted: run `PYTHONPATH=src python -m aief_gate`** |
...
| ID | Item | Blocks |
|---|---|---|
| ECR-D-001…004, 007…011 | **All dispositioned, approved, applied and registered** in session `S-2026-08-10-01`. Close on `C6`. **No longer blocking** — see [`.ai/project/OPEN_ITEMS.md`](.ai/project/OPEN_ITEMS.md) | Nothing |
...
> ### CAD is NOT yet authorised — `C6` FAILS
>
> **`LC-M04-EXIT` is not passed.** `C1`–`C5` and `C7` PASS; **`C6` FAILS** because both
> independent rounds — [`VER-015`](.ai/project/verification/VER-015_Independent_Verification_ECR-D-002_003_004_and_the_Coherence_Package.md)
> and [`VER-016`](.ai/project/verification/VER-016_Confirmatory_Round_On_VER-015_Repairs.md) —
> declare **NOT CLEARED**. Dispositions at [`DR-004`](.ai/project/reviews/DR-004_VER-015_Disposition.md).
> A confirmatory round on the last two repairs is owed before CAD starts.
>
> The design-authority *engineering* precondition is met. Session `S-2026-08-10-01` dispositioned
> **eight** ECRs against the frozen specification — `ECR-D-001`, `-002`, `-003`, `-004`,
> `-007`, `-008`, `-009`, `-010`, `-011` — each ruled by the human owner, applied to `spec/**`,
> re-registered in [`FROZEN.md`](.ai/project/FROZEN.md) and bound by approvals
> `APR-020`…`APR-026`. **All three CAD holds (H1 channel, H2 locators, H3 stubs and
> counterbores) are discharged.**
>
> **Start here:**
> [`implementation/01_SEWCP-200_Cooling_Plate/SEWCP-200_CAD_Implementation_Package.md`](implementation/01_SEWCP-200_Cooling_Plate/SEWCP-200_CAD_Implementation_Package.md).
> Read the superseded-in-part banner at its head first — the package is Rev X1, written against
> the pre-disposition baseline; its numeric errors are corrected in place and a full Rev X2
> re-issue is `OI-P-03`.
>
> **Verify before modelling. Nothing here is asserted; all of it computes:**
>
> ```
> PYTHONPATH=src python -m aief_gate          # LC-M04-EXIT C1..C7 from repository bytes
> PYTHONPATH=src python -m aief_approval verify   # approval-chain integrity
> PYTHONPATH=src python -m aief_clearance     # spec/00 s3.2 feature clearance
> PYTHONPATH=src python -m pytest tests/ -q
> ```
>
> Three defect classes that had gone unnoticed for many sessions were each a **declared property
> with no standing check**, and each now has one: approval supersession (`aief_approval`), the
> gate criteria themselves (`aief_gate`), and feature clearance (`aief_clearance`). The last of
> these found `ECR-D-010` — three choke stations colliding with three locators — which
> `spec/00` §3.2 had claimed *"No conflicts"* about while omitting the locators from its own map.
